 ...
 */
class WC_Fretebarato_Shipping_Simulator extends WC_Fretebarato
{
    /**
    ...
    public function enqueue_scripts()
    {
        ...
        $helper = new WC_Fretebarato_Helper;
        if (!$helper->is_simulator_enabled()) {
            return;
        }

        wp_enqueue_style('shipping-simulator', plugins_url('assets/css/simulator.css', plugin_dir_path(__FILE__)), array(), WC_Fretebarato_Main::VERSION, 'all');
        wp_enqueue_script('shipping-simulator', plugins_url('assets/js/simulator.js', plugin_dir_path(__FILE__)), array('jquery'), WC_Fretebarato_Main::VERSION, true);
        wp_localize_script(
            'shipping-simulator',
            'shipping_simulator',
            array(
    ...
    public static function simulator()
    {
        ...
        if ($product->is_in_stock() && in_array($product->get_type(), array('simple', 'variable', 'composite'))) {
            ...
            wc_get_template('single-product/shipping-simulator.php', array(
                ...
                'style' => $style,
                'ids' => $ids,
            ), '', WC_Fretebarato_Main::get_templates_path());
    ...
    public static function ajax_simulator()
    {
        ...
        $fretebarato = new WC_Fretebarato(sanitize_text_field($post['instance_id']));
        ...
        $fretebarato->quoteByProduct=true;
        $shippingValues = $fretebarato->fretebarato_calculate($package);

        // Adicionar flag display_date para o JavaScript
        $shippingValues['display_date'] = $fretebarato->get_option('display_date') === 'yes';
...
new WC_Fretebarato_Shipping_Simulator();
