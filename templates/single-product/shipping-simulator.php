...
foreach ($helper->get_instance_ids() as $object) {
    $fretebarato = new WC_Fretebarato($object->instance_id);
    $instance_id = $object->instance_id;
    $class_id = $product_shipping_class->term_id;
    $fretebarato_class_id = $fretebarato->get_option('shipping_class_id') ? (int)$fretebarato->get_option('shipping_class_id') : -1;

    if ($class_id === $fretebarato_class_id || (int)$fretebarato_class_id === -1) {
