                ...
                if (jQuery.isEmptyObject(response)) {
                    ...
                    jQuery.each(response, function (key, value) {
                        if (value.name !== undefined && value.price !== undefined) {
                            let days = parseInt(value.days || 0, 10);
                            let EstimatingDelivery = days + parseInt(additional_time, 10);

                            const shippingLi = document.createElement('li');
                            shippingLi.classList.add('li-fretebarato');

                            const shippingSpan = document.createElement('span');
                            shippingSpan.classList.add('span-fretebarato');
                            shippingSpan.innerText = value.name + ': ';

                            shippingLi.appendChild(shippingSpan);
                            shippingLi.innerText += 'R$' + parseFloat(value.price).toFixed(2);

                            if (response.display_date === true && days > 0) {
